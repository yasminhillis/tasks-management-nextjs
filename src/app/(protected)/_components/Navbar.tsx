...
export default function NavbarClient() {
  ...
  return (
    <>
      <header
        className={`font-sans z-90 flex items-center justify-between md:justify-end px-5 py-3 md:px-6 md:py-3 border-b border-b-[#0000001A] bg-[#F9F9FF] w-full`}
      >
        {logoutStatus && (
          <div
            className={`fixed bottom-5 right-5 max-w-sm px-4 py-3 md:bottom-6 md:right-6 max-md:bottom-16 max-md:right-0 max-md:mx-3 max-md:rounded-lg max-md:left-0 max-md:max-w-full
        ...
        <div className="flex items-center gap-2 md:gap-4">
          ...
          <div ref={dropDownRef} className="relative">
            <Initials
              ...
              extraStyles="rounded-md cursor-pointer bg-primary-container text-white w-10 h-10"
              state="success"
            />
            {isDropDownOpen && (
              <div className="absolute right-0 top-10 z-70 w-[200px] h-auto py-4 px-4 bg-white border border-gray-400 shadow-sm flex flex-col gap-4 justify-start rounded-lg">
                <div className="flex gap-2 items-center ">
                  ...
                  <div className="flex flex-col gap-1 justify-center items-center">
                    ...
                    <span className="text-[#003D9B] uppercase font-bold text-[10px] tracking-[1px]">
                      {department}
                    </span>
                  </div>
                </div>
                ...
                <div
                  onClick={logUserOut}
                  className="flex items-center gap-2 text-[#BA1A1A] hover:text-[#e72020] cursor-pointer font-medium"
                >
      ...
      {isDropDownOpen && (
        <div className="fixed inset-0 z-60  bg-[#041B3C66]/40 backdrop-blur-[1px]"></div>
      )}
    </>
  );
}

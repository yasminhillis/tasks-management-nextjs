...
export default function EpicsList({ projectId }: { projectId: string }){
    ...
    const { epics, error, loading } = useAppSelector(state => state.epics);
  ...
  if (loading === 'succeeded' && error.length === 0 && epics.length === 0) {
    return <EmptyState imageSrc="/emptyEpic.png" imageWidth={288} imageHeight={288} imageAlt="empty epic list" title="No epics in this project yet." description="Break down your large project into manageable
epics to track progress better and maintain
architectural clarity." imageStyles="" buttonLabel="Create First Epic" materialButtonIcon="electric_bolt" onButtonClick={() => router.push(`/project/${projectId}/epics/new`)}
    footer={
        <div className="grid grid-cols-1 gap-[15px] md:grid-cols-3 md:gap-[24px]">
            <FeatureHintCard materialIcon="auto_awesome" title="High Level Goals" description="Define the broad objectives
    that span across multiple
    cycles." />
    
    <FeatureHintCard materialIcon="schema" title="High Level Goals" description="Define the broad objectives
    that span across multiple
    cycles." />
    
    <FeatureHintCard materialIcon="timeline" title="High Level Goals" description="Define the broad objectives
    that span across multiple
    cycles." />
        ...
        <PageWrapper>
            <Header 
                ...
                <div className="flex items-center bg-[#D7E2FF] px-[12px] py-[6px] rounded-[2px]">
                    <span className="material-symbols-outlined" style={{'fontSize': '16px', 'color': '#737685'}}>search</span>
                    <input type="text" placeholder="Search epics..." className="text-[14px] text-[#737685] px-[12px] py-[10px] outline-none"/>
                </div>
                }
            />
...
again in a moment.`} buttonElement/>
  }

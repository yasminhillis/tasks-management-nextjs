...
export default function MembersTable({ data }: MembersTableProps) {
  ...
  const memberElementsDesktop = data.map((member: MemberData) => (
    <tr key={member.member_id}>
      ...
      <td className="px-8 py-5 align-middle">
        <Pill mode="desktop" role={member.role} extraStyles='rounded-lg' state="success" />
      </td>
  ...
  const memeberElementsMobile = data.map((member: MemberData) => (
    <li key={member.member_id} className='flex items-center justify-between bg-white rounded-lg p-4'>
      <div className="flex gap-4">
        <Initials mode="mobile" state='success' name={member.metadata.name} extraStyles="w-10 h-10 rounded-lg bg-[#D7E2FF] flex items-center justify-center text-[16px] font-bold text-[#003D9B]" />
        <div>
          <h3 className="body-sm">{member.metadata.name}</h3>
      ...
      <div className='flex flex-col gap-2'>
        <Pill mode="mobile" state='success' role={member.role} extraStyles="text-[10px] font-bold tracking-[-0.25px] uppercase text-[#434654] bg-[#D7E2FF] rounded-[2px] px-2 py-1"/>
        <div className="flex justify-end">
          <span className="material-symbols-outlined">more_vert</span>
        </div>
      </div>
  ...
  return (
    <>
      <table className="hidden md:table table-fixed w-full rounded-md overflow-hidden p-1 shadow-sm bg-white">
        ...
        <tbody className="divide-y divide-[#E8EDFF]">{memberElementsDesktop}</tbody>
      </table>

      <ul className='md:hidden mb-8'>
        {memeberElementsMobile}
      </ul>

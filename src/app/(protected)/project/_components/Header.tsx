...
export default function Header({
  ...
  if (loading) {
      ...
      <div className="hidden md:block md:flex items-center justify-between mb-10">
        <div className={`hidden md:block w-[256px] h-[40px] rounded-[4px] ${shimmer}`}>
          
        </div>
        <div className="flex gap-4 items-center">
          {searchBar && <div className={`w-[128px] h-[40px] rounded-[2px] ${shimmer}`}></div>}
          
          <div className={`hidden md:block w-[160px] h-[40px] rounded-[2px] ${shimmer}`}>
          </div>
        </div>
      </div>
    </div>

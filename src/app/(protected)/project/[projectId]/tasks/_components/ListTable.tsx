...
//       "created_by": {"id": "031b7dc1-326c-4f32-b0a6-bf2267b0a8ef", "name": "Yasmin Ayman", "email": "yasminhillis7@gmail.com", "department": "Frontend"},
//       "description": "Redesign the task detail modal to match new spec",
//       "due_date": "2026-08-30T21:00:00+00:00",
//       "epic": {"id": "9d0e1f2a-3b4c-4d5e-8f6a-7b8c9d0e1f2a", "title": "UI Refresh Q3", "epic_id": "EPIC-2"},
...
//       "created_by": {"id": "5b6c7d8e-9f0a-4b1c-8d2e-3f4a5b6c7d8e", "name": "Laila Ahmed", "email": "laila.ahmed@example.com", "department": "Design"},
...
//   ]
  ...
  const statusColor = {
    'IN PROGRESS': {bg: '#CDDDFF', text: '#1D4ED8'}, 
    'TO DO': {bg: '#D7E2FF', text: '#475569' }, 
    ...
    <table className="rounded-lg shadow-card">
      <thead className="bg-[#F1F3FF80] border-b border-b-[#C3C6D61A] h-[47px] w-full">
        <tr className="text-left">
          ...
          <th className="table-header-label px-6 py-[18.5px]">Status</th>
          ...
          tasks.map((task) => (
            <tr key={task.id} className="border-t border-t-[#F1F3FF]">
              ...
              <td className="w-[137px] px-6 py-[18.5px]">
                <div className="table-status-badge status-pill bg-blue-300 whitespace-nowrap" style={{'backgroundColor': statusColor[task.status.replaceAll('_', ' ')]?.bg, 'color': statusColor[task.status.replaceAll('_', ' ')]?.text}}>
              ...
              <td className="table-cell-assignee w-[163px] px-6 py-[18.5px] whitespace-nowrap">
                {task.assignee.name ? (
                  ...
                  <div className="flex items-center gap-3">
                    <span
                      className="rounded-full w-[28px] h-[28px] text-[#4F5F7B] bg-[#E8EDFF] inline-flex justify-center items-center"

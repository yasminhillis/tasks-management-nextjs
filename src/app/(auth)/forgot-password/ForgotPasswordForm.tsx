'use client'
import { type ForgotPasswordFormData, ForgotPasswordSchema } from "@/validations/forgot.password.schema";
...
export default function ForgotPasswordForm(){
    ...
    function startTimer(){
        ...
        intervalRef.current = setInterval(() => {
            if (time <= 0) {
                ...
                let seconds = time % 60; 
                setShowTime(`${String(minutes).padStart(2, '0')}:${String(seconds).padStart(2, '0')}`)
                time -= 1
            }
        }, 1000)
    }
    ...
    return (
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="bg-white p-8 font-sans rounded-md mb-6 shadow-[0_4px_12px_rgba(0,0,0,0.1)]">
            ...
            <h3 className="font-semibold text-2xl text-center text-slate-900 mb-2">Forgot password?</h3>
            <p className="text-[#434654] text-sm text-center w-64 mx-auto mb-8">No worries, we'll send you reset instructions.</p>
            ...
            <Link className="cursor-pointer flex items-center justify-center text-center text-[#003D9B] hover:text-[#2b76e8] transition-colors mx-auto text-sm font-medium gap-2" href="/login">
        ...
        { 
            showSuccessMessage && (<div className="flex flex-col w-[342px]  bg-[#82F9BE]/30 text-[#005235] backdrop-blur-md rounded-sm p-4 border border-[#0052351A] font-sans mb-10">
                <div className="flex gap-3 mb-3">
                    <span className="material-symbols-outlined items-center" style={{ fontVariationSettings: "'FILL' 1" }}>
                        check_circle
                    </span>
                    <p className="text-[#005235] font-sans text-xs">If an account exists with this email, we've sent
                ...
                <div className="flex items-center justify-between">
                    <p  className="uppercase font-bold text-[11px] text-[#00523599] ">Didn't receive email?</p>
                    <button disabled={!enableResendButton || resendTrialsLeft === 0} onClick={() => resendEmail()} className="text-[#003D9B] uppercase text-[11px] tracking-[1.1px] font-bold cursor-pointer disabled:cursor-not-allowed">
